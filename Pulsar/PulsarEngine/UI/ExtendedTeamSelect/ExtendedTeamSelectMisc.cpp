...
void CtrlRaceNameBalloon_refresh(CtrlRaceNameBalloon *_this, u8 playerId) {
    ...
    if (ExtendedTeamManager::IsActivated()) {
        ...
        if (static_cast<Pulsar::ExtendedTeamsLine>(Pulsar::Settings::Mgr::Get().GetUserSettingValue(Pulsar::Settings::SETTINGSTYPE_EXTENDEDTEAMS, Pulsar::RADIO_EXTENDEDTEAMSLINE)) == Pulsar::EXTENDEDTEAMS_LINE_TEAM) {
            for (int i = 0; i < 4; i++) {
